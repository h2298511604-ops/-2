# app.py – V7.2.0-hotfix1  强制刷新门店数量（符号已修复）
from flask import Flask, request, jsonify
from flask_cors import CORS
from dotenv import load_dotenv
import os
from services.amap_client import (
    calculate_commute, search_nearby_refined, regeo_address, get_input_tips
)
from services.scoring import (
    calculate_rent_income_score, score_hardware,
    score_house_layout, calculate_location_refined_score
)
...
def input_tips():
    ...
    return jsonify([{'name': t.get('name', ''), 'district': t.get('district', '')}
                    for t in tips if t.get('name')])
...
def calculate_score():
    try:
        ...
        # 4. 硬件配套
        h_score = score_hardware(data['house_info']['has_bathroom'],
                                 data['hardware']['appliances'])

        # 5. 户型结构
        layout_res = score_house_layout(data['house_info'])

        total = (income_score * 0.25 + h_score * 0.25 + c_score * 0.20 +
                 loc_res['score'] * 0.20 + layout_res['score'] * 0.10)

        return jsonify({
            'total_score': round(total, 1),
            'details': {
                '收入租金比': {'score': round(income_score, 1),
                             'desc': f"租金占收入 {(rent/income*100):.1f}%"},
                '硬件配套': {'score': round(h_score, 1),
                           'desc': "基础家电与卫生间评估"},
