// src/App.jsx – V7.2.0-hotfix1（仅追加调试日志，无逻辑变更）
import React, { useState, useCallback, useMemo } from 'react';
import { Layout, Form, Input, Button, Card, Row, Col, message, Tag, Select, AutoComplete, Checkbox, InputNumber, Switch } from 'antd';
...
function App() {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      try {
        ...
        if (res.data.address) {
          ...
          message.success('定位成功并锁定坐标');
        }
      } catch { hide(); message.error('定位转换失败'); }
    }, () => { hide(); message.error('请授予位置权限'); });
  };
  ...
  const onFinish = async (values) => {
    ...
    try {
      const appliances = {};
      APPLIANCE_OPTIONS.forEach(a => appliances[a] = values.appliances?.includes(a) || false);
      const payload = {
        ...values,
        city: currentCity,
      ...
      /* ===== 调试日志开始 ===== */
      console.log('[DEBUG] 周边配套 counts.life =', res.data.details['周边配套']?.counts?.life);
      console.log('[DEBUG] 品牌数量 =', res.data.details['周边配套']?.brands?.length);
      /* ===== 调试日志结束 ===== */
    } catch { message.error('评估出错，请确认地址为下拉点选'); }
              ...
              <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ city: '南宁', rent: 2000, income: 6000, commute_mode: 'subway', rent_type: 'solo', people: 1, floor: 5, elevator: true, bathroom: true }}>
                <Row gutter={12}>
                  ...
                  <Col span={12}>
                    <Form.Item label="通勤方式" name="commute_mode" rules={[{ required: true }]}>
                      <Select>
                        <Option value="subway" disabled={!hasSubway}>🚇 地铁 {!hasSubway && '(未开通)'}</Option>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <Form.Item name="rental_address" rules={[{ required: true, message: '必选' }]} style={{ flex: 1 }}>
                      <AutoComplete options={rentalTips} onSearch={(v) => fetchTips(v, 'rental', currentCity)} onSelect={(val, opt) => setRentalCoords(opt.location)}>
                        <Input prefix={<EnvironmentOutlined />} placeholder="输入房源..." allowClear />
                      </AutoComplete>
                    </Form.Item>
                ...
                <Form.Item label="公司地址 (下拉点选)" name="company_address" rules={[{ required: true }]}>
                  <AutoComplete options={companyTips} onSearch={(v) => fetchTips(v, 'company', currentCity)} onSelect={(val, opt) => setCompanyCoords(opt.location)}>
                    <Input prefix={<CarOutlined />} placeholder="输入公司..." allowClear />
                  </AutoComplete>
                </Form.Item>
                <Row gutter={12}>
                    ...
                    <div key={k} style={{ marginBottom: 15, borderBottom: '1px dashed #eee', paddingBottom: 10 }}>
                      ...
                      {k === '周边配套' && (
                        <div style={{ marginTop: 10, background: '#fff', border: '1px solid #f0f0f0', padding: 10, borderRadius: 6 }}>
                          ...
                          <div style={{ fontWeight: 'bold', fontSize: 11, marginBottom: 5 }}>🔥 品牌清单 (离住处由近到远)：</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {v.brands?.slice(0, 5).map((b, i) => <div key={i} style={{ fontSize: 11, color: '#444' }}>· {b}</div>)}
                            {v.brands?.length > 5 && <div style={{ fontSize: 11, color: '#999', paddingLeft: 8, fontStyle: 'italic' }}>等等...</div>}
                          </div>
                        </div>
                      )}
                    </div>
